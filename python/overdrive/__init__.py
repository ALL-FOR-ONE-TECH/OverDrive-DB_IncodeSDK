...
class _Native:
    ...
    def __init__(self):
        ...
        # Lifecycle
        # Use c_size_t (unsigned, pointer-width) for all ODB handle returns.
        # c_void_p on 64-bit Windows sign-extends pointers above 2 GB → access violation.
        _H = ctypes.c_size_t  # opaque ODB* handle type
        self._handle_type = _H

        self.lib.overdrive_open.argtypes = [ctypes.c_char_p]
        self.lib.overdrive_open.restype = _H

        self.lib.overdrive_close.argtypes = [_H]
        self.lib.overdrive_close.restype = None

        self.lib.overdrive_sync.argtypes = [_H]
        self.lib.overdrive_sync.restype = None

        # Tables
        self.lib.overdrive_create_table.argtypes = [_H, ctypes.c_char_p]
        self.lib.overdrive_create_table.restype = ctypes.c_int

        self.lib.overdrive_drop_table.argtypes = [_H, ctypes.c_char_p]
        self.lib.overdrive_drop_table.restype = ctypes.c_int

        self.lib.overdrive_list_tables.argtypes = [_H]
        self.lib.overdrive_list_tables.restype = ctypes.c_size_t  # char*

        self.lib.overdrive_table_exists.argtypes = [_H, ctypes.c_char_p]
        self.lib.overdrive_table_exists.restype = ctypes.c_int

        # CRUD
        self.lib.overdrive_insert.argtypes = [_H, ctypes.c_char_p, ctypes.c_char_p]
        self.lib.overdrive_insert.restype = ctypes.c_size_t  # char* — must be freed

        self.lib.overdrive_get.argtypes = [_H, ctypes.c_char_p, ctypes.c_char_p]
        self.lib.overdrive_get.restype = ctypes.c_size_t  # char* — must be freed

        self.lib.overdrive_update.argtypes = [_H, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p]
        self.lib.overdrive_update.restype = ctypes.c_int

        self.lib.overdrive_delete.argtypes = [_H, ctypes.c_char_p, ctypes.c_char_p]
        self.lib.overdrive_delete.restype = ctypes.c_int

        self.lib.overdrive_count.argtypes = [_H, ctypes.c_char_p]
        self.lib.overdrive_count.restype = ctypes.c_int

        # Query
        self.lib.overdrive_query.argtypes = [_H, ctypes.c_char_p]
        self.lib.overdrive_query.restype = ctypes.c_size_t  # char* JSON

        self.lib.overdrive_search.argtypes = [_H, ctypes.c_char_p, ctypes.c_char_p]
        self.lib.overdrive_search.restype = ctypes.c_size_t  # char* JSON

        # Utility
        self.lib.overdrive_last_error.argtypes = []
        self.lib.overdrive_last_error.restype = ctypes.c_char_p

        self.lib.overdrive_free_string.argtypes = [ctypes.c_size_t]
        self.lib.overdrive_free_string.restype = None
        ...
        # Phase 5: MVCC Transactions
        self.lib.overdrive_begin_transaction.argtypes = [_H, ctypes.c_int]
        self.lib.overdrive_begin_transaction.restype = ctypes.c_uint64

        self.lib.overdrive_commit_transaction.argtypes = [_H, ctypes.c_uint64]
        self.lib.overdrive_commit_transaction.restype = ctypes.c_int

        self.lib.overdrive_abort_transaction.argtypes = [_H, ctypes.c_uint64]
        self.lib.overdrive_abort_transaction.restype = ctypes.c_int
    ...
    def _bind_optional_symbols(self):
        ...
        if self._optional_bound:
            return
        self._optional_bound = True
        _H = self._handle_type
        try:
            self.lib.overdrive_verify_integrity.argtypes = [_H]
            self.lib.overdrive_verify_integrity.restype = ctypes.c_size_t
        except AttributeError:
            self.lib.overdrive_verify_integrity = None
        try:
            self.lib.overdrive_stats.argtypes = [_H]
            self.lib.overdrive_stats.restype = ctypes.c_size_t
        except AttributeError:
            self.lib.overdrive_stats = None
        ...
        self.lib.overdrive_open_with_engine.restype = _H  # ODB handle
        ...
        self.lib.overdrive_open_with_password.restype = _H  # ODB handle
...
class OverDrive:
    ...
    def __init__(self, path: str):
        self._native = _get_native()
        # Always store absolute path — relative paths break when CWD changes
        # (e.g., backup() uses shutil.copy2 which requires a resolvable path)
        self._path = str(Path(path).resolve())
        self._handle = self._native.lib.overdrive_open(self._path.encode("utf-8"))
        if not self._handle:
            _check_error(self._native)
            raise OverDriveError(f"Failed to open database: {path}")
    ...
    def open(
        ...
        instance._handle = handle
        # Always store absolute path — relative paths break when CWD changes
        instance._path = str(Path(path).resolve())
        _set_secure_permissions(str(Path(path).resolve()))
        return instance
    ...
    def backup(self, dest_path: str) -> None:
        ...
        self._ensure_open()
        self.sync()
        # Resolve both paths to absolute to avoid CWD-dependent failures
        src = self._path  # already absolute (set in __init__)
...
>>>>>>> e9ee1f9 (fix: v1.4.3 - fix all SDK packages, Go no-CGo on Windows, CI/CD overhaul)
