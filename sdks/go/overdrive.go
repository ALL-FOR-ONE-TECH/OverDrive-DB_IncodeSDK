...
// getPlatformDir returns the platform-specific directory path for the new native/ structure.
...
func loadLibrary() (libHandle, error) {
	...
	// NEW: Add centralized native/ directory paths
	platformDir := getPlatformDir()
	
	searchDirs := []string{
		// 1. NEW STRUCTURE: Centralized native libraries
		filepath.Join(cwd, "..", "..", "native", platformDir),
		filepath.Join(cwd, "..", "..", "native", "windows"), // Windows fallback
