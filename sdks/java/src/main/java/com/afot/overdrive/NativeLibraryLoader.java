...
public class NativeLibraryLoader {
    ...
    private static OverDrive.LibOverDrive tryLoadBundled(String libraryName, String platform) {
        try {
            ...
            // NEW STRUCTURE: Try centralized native/ directory first
            // Path: /native/{platform}/{library} (e.g., /native/windows/overdrive.dll)
            String newResourcePath;
            if ("windows".equals(os)) {
                newResourcePath = "/native/windows/" + libraryName;
            } else if ("macos".equals(os)) {
                if ("arm64".equals(arch)) {
                    newResourcePath = "/native/macos/arm64/" + libraryName;
                } else {
                    newResourcePath = "/native/macos/x64/" + libraryName;
                }
            } else { // linux
                if ("arm64".equals(arch)) {
                    newResourcePath = "/native/linux/arm64/" + libraryName;
                } else {
                    newResourcePath = "/native/linux/x64/" + libraryName;
                }
            }
            
            // Try to find the library in new centralized structure
            InputStream resourceStream = NativeLibraryLoader.class.getResourceAsStream(newResourcePath);
            
            if (resourceStream == null) {
                // BACKWARD COMPATIBILITY: Try old organized structure
                String oldResourcePath = "/native/" + os + "/" + arch + "/" + libraryName;
                resourceStream = NativeLibraryLoader.class.getResourceAsStream(oldResourcePath);
                ...
                if (resourceStream == null) {
                    ...
                    if (resourceStream == null) {
                        // Library not found in any location
                        return null;
                    }
                }
            }
